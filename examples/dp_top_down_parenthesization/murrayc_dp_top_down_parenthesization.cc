...
private:
  ...
  calc_subproblem(type_level level, uint i, uint j) const override {
    ...
    if (j == i) {
      ...
      // indent(level);
      // std::cout << "returning base cost: " << result.cost << " for: " <<
      // result.dimensions.to_string() << std::endl;
      return result;
    }
    ...
    result.dimensions =
      calc_dimensions(sub_i_to_k.dimensions, sub_k_to_j.dimensions);
    result.solution = "(" + sub_i_to_k.solution + sub_k_to_j.solution + ")";

    // indent(level);
    // std::cout << "returning cost: " << result.cost << " for: " <<
    // result.dimensions.to_string() << std::endl;
    return result;
  }
  ...
  static uint
  calc_cost(const MatrixDimensions& a, const MatrixDimensions& b) {
    // a p x q matrix multiplied by a q x r makes a p x r matrix,
    // and is only possible if r == q.
    if (a.columns_count != b.rows_count) {
      std::cout << "calc_cost: Invalid dimensions for " << a.to_string()
                << " and " << b.to_string()
                << ": a.columns_count=" << a.rows_count
                << ", b.rows_count=" << b.columns_count << std::endl;
  ...
  static MatrixDimensions
  calc_dimensions(const MatrixDimensions& a, const MatrixDimensions& b) {
...
int
main() {
  const VecMatrixDimensions vec({{"A", 10, 30}, {"B", 30, 5}, {"C", 5, 60}});

  DpParenthesization dp(vec);
  const auto result = dp.calc();
  std::cout << "result: " << result.cost << ": " << result.solution
            << std::endl;
