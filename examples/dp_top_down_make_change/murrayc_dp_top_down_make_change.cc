...
class DpMakeChange : public murraycdp::DpTopDownBase<SubSolution,
                       SubSolution::type_size, SubSolution::type_value> {
public:
  ...
  : items_(items), needed_value_(needed_value) {}

private:
  type_subproblem
  calc_subproblem(type_level level, type_size item_number,
    type_value needed_value) const override {
    // indent(level);
    // std::cout << "calc_subproblem(): item_number = " << item_number << ",
    ...
    auto case_dont_use_this_item =
      (item_number == 1 ? SubSolution(SubSolution::COIN_COUNT_INFINITY)
    ...
    if (item_value == needed_value) {
      ...
      // indent(level);
      // std::cout << "Taking case_dont_use_this_item because item_value too
      // big: " << item_value << std::endl;
      ...
      // indent(level);
      // std::cout << "case_use_this_item=" <<
      // case_use_this_item.coin_count_used <<
      ...
      // The normal knapsack problem would need us to check for a maximum
      // instead,
      // to maximize value.
    ...
    // indent(level);
    // std::cout << "item_value=" << item_value << ", returning result:
    // coin_count_used=" << result.coin_count_used << std::endl;
    return result;
  }

  void
  get_goal_cell(
    type_size& item_number, type_value& needed_value) const override {
...
*/
  ...
  if (result.coin_count_used != SubSolution::COIN_COUNT_INFINITY) {
    std::cout << "solution: coins count: " << result.coin_count_used
              << std::endl
              << "with solution: ";
