...
using type_subset_iterator = SubsetIterator<type_vec_node_ids>;

//TODO: Problems:
//- The key for the subset is the subset_id, but we iterate over the
//subset_iterator (which can give us a subset, which can give us an ID).
//- The subset_iterator start and end depends on the outer m value.
//- The j values (not just a simple start and end) depend on the IDs in the subset referenced by the subset_iteror.
class DpTsp
  : public murraycdp::DpBottomUpBase<
      2 /* count of subproblems to keep, used in calc_subproblem() */,
      type_subproblem,
      std::size_t, // m: nodes_count
      const type_subset_iterator&,
      std::size_t // j: each node id in the subset.
      > {
public:
  DpTsp(const type_vec_nodes& vertices)
  : DpBottomUpBase(vertices.size(), type_subset_iterator::end(), 99 /* TODO */), //DpBottomUpBase without the specialization is apparently allowed.
    vertices_(vertices),
  ...
  type_subproblem calc() override {
    ...
    for(const auto j : vertex_ids_) {
      ...
      const type_subset_iterator iter_full =
        type_subset_iterator::create(full_subset.size(), full_subset.size());

      const auto length = get_subproblem(level, i, iter_full, j);
...
private:
  ...
  type_subproblem calc_subproblem(type_level level, std::size_t m, const type_subset_iterator& subset_iter, std::size_t j) const override {
    if(j == start_node_id_)
      return 0; //TODO
      
    const auto subset = subset_iter.get_subset(vertex_ids_); //TODO: Is vertex_ids_ the correct set here?
  ...
  void get_goal_cell(std::size_t& /* m */, type_subset_iterator& /* subset_iter */, std::size_t& /* j */) const override {
     //We don't use this.
