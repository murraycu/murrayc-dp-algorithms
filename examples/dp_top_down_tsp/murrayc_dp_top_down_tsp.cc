...
public:
  ...
  Vertex(type_length x_in, type_length y_in) : x(x_in), y(y_in) {}
...
static inline type_length
calc_distance(const Vertex& a, const Vertex& b) {
...
static inline type_length
get_sub_problem_base_case(
  const type_vec_node_ids& subset, type_node_id start_node_id) {
  // Base case:
...
static inline type_subset_id
get_subset_id(const type_vec_node_ids& vertex_ids) {
...
struct hash<type_vec_node_ids> {
  size_t
  operator()(const type_vec_node_ids& value) const {
    // TODO: Performance:
...
class DpTsp : public murraycdp::DpTopDownBase<type_subproblem,
...
public:
  ...
  calc() override {
    ...
    for (const auto j : vertex_ids_) {
      ...
      const auto Cj1 = calc_distance(vertices_[j], start_node_);
      // std::cout << "Cj1=" << Cj1 << std::endl;
      const auto full_length = length + Cj1;

      // std::cout << "  : j=" << j << ", full_length=" << full_length <<
      // std::endl;
...
private:
  ...
  type_subproblem
  calc_subproblem(type_level level, std::size_t m,
    const type_vec_node_ids& subset, std::size_t j) const override {
    if (j == start_node_id_)
      return 0; // TODO
    ...
    // std::cout << ": j=" << j << ", subset_without_j = {" <<
    // get_subset_id(subset_without_j) << "}" << std::endl;

    for (const auto k : subset) {
      if (k == j)
        continue;

      // std::cout << "  j= " << j << ", k=" << k << ", subset_without_j = {" <<
      // get_subset_id(subset_without_j) << "}" << std::endl;
      ...
      const auto Ckj = calc_distance(vertices_[k], vertices_[j]);
      // std::cout << "  using A[{" << subset_without_j_id << "}, " << k << "] =
      // " << length << std::endl;
  ...
  void
  get_goal_cell(std::size_t& /* m */, type_vec_node_ids& /* subset */,
    std::size_t& /* j */) const override {
    // We don't use this.
    // Instead we override calc() and get a minimum over several cells.
  }
...
bool
doubles_are_equal(double a, double b) {
...
*/
  ...
  const type_vec_nodes vertices = {{0.328521, 0.354889}, {0.832, 0.832126},
    {0.680803, 0.865528}, {0.734854, 0.38191}, {0.14439, 0.985427},
    {0.90997, 0.587277}, {0.408464, 0.136019}, {0.896868, 0.916344},
    {0.991904, 0.383134}, {0.451197, 0.741267}, {0.825205, 0.761446},
    {0.421804, 0.0374936}, {0.332503, 0.26436}, {0.107117, 0.51559},
    {0.845227, 0.21359}, {0.880095, 0.593086}, {0.454773, 0.834355},
    {0.7464, 0.363176}};
