...
#ifndef MURRAYCDP_TUPLE_HASH_H
#define MURRAYCDP_TUPLE_HASH_H
...
inline void
hash_combine(std::size_t& seed, const T_tuple& v) {
  seed ^=
    hash_tuple::hash<T_tuple>()(v) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
}
...
public:
  size_t
  operator()(const std::tuple<T_elements...>& tt) const {
    size_t seed = 0;
